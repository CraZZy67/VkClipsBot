...
class UserAuthorizer:
    ...
    def save_session_creds(self):
        self.driver.get(self.LOGIN_LINK)
        
        cookie = self.driver.get_cookies()
        creds = self.driver.execute_script(f'return localStorage.getItem("{self.LOCAL_STORAGE_KEY}");')
        
        load_dotenv()
        creds_path = os.getenv('CREDS_PATH')
        
        with open(creds_path + 'user_creds.pickle', 'wb') as file:
